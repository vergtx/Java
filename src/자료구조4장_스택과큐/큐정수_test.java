...
class Queue {
	...
	/*
	 * i는 반복문에서 현재 순회하고 있는 인덱스를 나타내며, front는 큐의 시작 위치를 나타냅니다. (i + front)는 현재 인덱스가
	 * 시작 위치를 기준으로 얼마나 떨어져 있는지를 나타내는 값 % capacity 연산을 수행하여 배열의 크기로 나눈 나머지를 계산하는 것
	 * 배열의 순환을 구현하는 핵심. 배열의 순환을 처리하면서 요소에 접근할 수 있습니다. 이 연산은 배열의 크기를 초과하지 않으면서 순환 구조를
	 * 구현하는 효과를 갖습니다.
	 */
